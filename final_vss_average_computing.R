    ...
    for(fs in c(10,20,30,40,50,60,70,80)){
      for(be in c(0.001)){
        for(al in c(0.5)){
          ...
          #存放模型数据文件
          tempfile = paste0("temp_data/avg_temp_alpha",al,"_beta",be,"_features",features,"_nnumber",nnumber,".RData")
          ...
          for(j in 1:features){
            for(iter in 1:maxIter){
              for(i in 1:length(nozero_inv)){
                ...
                psum = 1 - 1/(1 - exp(-sum((gene_features_dist_avg_reg[inv$gene,] - phene_features_dist_avg_reg[inv$phene,])^2)/2 + lambda))
                error = inv$value + (1 - inv$value) * psum
                
                #更新特征
                
                temp1 = gene_features_dist_avg_reg[inv$gene,j]#暂存，用于更新项目特征
                temp2 = phene_features_dist_avg_reg[inv$phene,j]
                #用户特征
                ...
                if(is.null(gene_nbs[inv$gene][[1]])){
                  ...
                  if(length(order) >=min_nbs){
                    ...
                    sim = sim / sum(sim)#计算平均异构相似约束时， 相似度归一化
                ...
                if(!is.null(nbs$order) & length(nbs$order)>0)
                  gradient = gradient - alpha * (temp1 - sum(nbs$sim * gene_features_dist_avg_reg[nbs$order,j])) #约束项
                ...
                if(TRUE){
                  ...
                  if(!is.null(nbs$order) & length(nbs$order)>0)
                    gradient = gradient - alpha2 * (temp1 - sum(nbs$sim * gene_features_dist_avg_reg[nbs$order,j])) #约束项
                }
                
                gene_features_dist_avg_reg[inv$gene,j] = temp1 + 
                  theta * gradient
                #项目特征
                ...
                if(!is.null(nbs$order) & length(nbs$order)>0)
                  gradient = gradient - beta * (temp2 - sum(nbs$sim * phene_features_dist_avg_reg[nbs$order,j]))
                phene_features_dist_avg_reg[inv$phene,j] = temp2 + 
                  theta * gradient
              }
            }
          }
          ...
          #计算模型预测值
          temp = sapply(1:nrow(gene_features_dist_avg_reg), function(x){
            sapply(1:nrow(phene_features_dist_avg_reg), function(y){
              1/exp(sum((gene_features_dist_avg_reg[x,] - phene_features_dist_avg_reg[y,])^2)/2 + lambda)
            })
          })
